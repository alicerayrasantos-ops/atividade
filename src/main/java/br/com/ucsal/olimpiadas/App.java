...
public class App {
	...
	static ProvaService provaService;
	static QuestaoService questaoService;
	static TentativaService tentativaService;

	public App(ParticipanteService participanteService, ProvaService provaService, QuestaoService questaoService, TentativaService tentativaService){
		App.participanteService = participanteService;
		App.provaService = provaService;
		App.questaoService = questaoService;
		App.tentativaService = tentativaService;
	}
	...
	static void cadastrarParticipante() {
		...
		Participante p = participanteService.CadastrarParticipante(nome, email, participantes, proximoParticipanteId++);
	...
	static void cadastrarProva() {
		...
		Prova prova = provaService.cadastrarPergunta(titulo, provas, proximaProvaId++);
		System.out.println("Prova criada: " + prova.getId());
	}

	private static String lerEnunciado() {
		System.out.println("Enunciado:");
		return in.nextLine();
	}

	private static String[] lerAlternativas() {
	...
	private static Character lerAlternativaCorreta() {
		System.out.print("Alternativa correta (A–E): ");

		try {
			return Questao.normalizar(
					in.nextLine().trim().charAt(0)
			);
		} catch (Exception e) {
	...
	static void cadastrarQuestao() {
		...
		String enunciado = lerEnunciado();
		String[] alternativas = lerAlternativas();
		Character correta = lerAlternativaCorreta();
		Questao q = questaoService.cadastrarQuestao(provas, provaId, alternativas, proximaQuestaoId++, enunciado, correta, questoes);

		System.out.println("Questão cadastrada: " + q.getId() + " (na prova " + provaId + ")");
	}

	static boolean validarCadastro() {
	...
	static List<Questao> buscarQuestoesDaProva(Long provaId) {
		return questoes.stream()
				.filter(q -> q.getProvaId() == provaId)
				.toList();
	}

	static void aplicarQuestoes(List<Questao> questoesDaProva, Tentativa tentativa) {
		for (var q : questoesDaProva) {
			Resposta resposta = aplicarQuestao(q);
			tentativa.getRespostas().add(resposta);
		}
	}
	static Resposta aplicarQuestao(Questao q) {
		System.out.println("\nQuestão #" + q.getId());
		...
		char marcada = lerResposta();

		var resposta = new Resposta();
		resposta.setQuestaoId(q.getId());
		resposta.setAlternativaMarcada(marcada);
		resposta.setCorreta(q.isRespostaCorreta(marcada));
	...
	static char lerResposta() {
		...
		try {
			return Questao.normalizar(in.nextLine().trim().charAt(0));
		} catch (Exception e) {
			System.out.println("resposta inválida (marcando como errada)");
			return 'X';
		}
	}
	static void finalizarTentativa(Tentativa tentativa) {
	...
	static void aplicarProva() {
		...
		var questoesDaProva = buscarQuestoesDaProva(provaId);
		if (questoesDaProva.isEmpty()) {
			System.out.println("esta prova não possui questões cadastradas");
			return;
		}

		Tentativa tentativa = tentativaService.criarTentativa(proximaTentativaId++, participanteId, provaId);

		System.out.println("\n--- Início da Prova ---");

		aplicarQuestoes(questoesDaProva, tentativa);

		finalizarTentativa(tentativa);
	}
